...
local Dialog = Window:Dialog({
    ...
    Text = "Hey! You pressed play on DarkDrigon, enjoy! ", 
...
local tab1 = Window:MakeTab({"information", "info"})
tab1:AddSection("Discord group")
...
tab1:AddParagraph({
"Updates:",
"English version for audiences outside of Brazil."
})
...
tab2:AddSection("player options")
tab2:AddSlider({
Title = "WalkSpeed",
MinValue = 16,
MaxValue = 200,
...
local toggleCallback = tab2:GetToggle("Activate ESP").Callback
toggleCallback(true)
end
end
})
...
if getgenv().SelectedESP == "ESP Red" then    
        ...
        while getgenv().RedESP and getgenv().ESPEnabled do    
            for _,v in pairs(game.Players:GetPlayers()) do    
                if v ~= game.Players.LocalPlayer and v.Character and not v.Character:FindFirstChild("RED_ESP") then    
                    ...
                    if getgenv().ESPMode == "ESP Name" then  
                        hl.Enabled = false  
                    end  
                    ...
                    if head then    
                        ...
                        if getgenv().ESPMode == "ESP Body" then  
                            gui.Enabled = false  
                        end  
...
elseif getgenv().SelectedESP == "ESP Green" then    
...
elseif getgenv().SelectedESP == "ESP Purple" then
...
elseif getgenv().SelectedESP == "ESP Yellow" then
...
local Dropdown = Tab:AddDropdown({
    Name = "Select a player to copy their skin",
...
Tab:AddButton({
    Name = "Copy the skin of the selected player",
    Callback = function()
        if not Target then 
        ...
        if TPlayer and TPlayer.Character then
            ...
            if LHumanoid and THumanoid then
                ...
                game:GetService("StarterGui"):SetCore("SendNotification", {
                    Title = "Skin copied successfully! ",
                    Text = "copied skin" .. Target .. "!",
                    Duration = 6
                })
            end
        else
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "error ",
                Text = "The player left the game or doesn't have a character; try again.",
                Duration = 5
            })
        end
    end
...
Tab:AddSection({"3D Clothing / 3D accessories"})
...
Troll:AddButton({
  Name = "FLING DOOR CLICK ",
Description = "STAY CLOSE TO THE DOORS",
  Callback = function()
local Players = game:GetService("Players")
...
Troll:AddButton({
    Name = "FLING ELIMINATING SOFA CLICK ",
    Callback = function()
...
local kill = Troll:AddSection({Name = "FLING ALL"})

Troll:AddButton({
    Name = "Eliminate all BUS",
    Callback = function()
        local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
local RunService = game:GetService("RunService")
...
local kill = Troll:AddSection({Name = "FLING BLACK HOLE "})
...
local kill = Troll:AddSection({Name = "LAGAR SERVER "})
...
local kill = Troll:AddSection({Name = "MENU SKY"})

Troll:AddButton({
    Name = "CHANGE THE SKY ",
    Description = "PLACE AN IMAGE IN THE SKY ",
    Callback = function()
        local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
Troll:AddButton({
    Name = "DISABLE ",
    Description = "REMOVE IMAGE FROM THE SKY ",
    Callback = function()
        local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
local TabNuke = Window:MakeTab({"Troll Map", "Map"})

TabNuke:AddSection({"Nuke"})
TabNuke:AddParagraph({
	"NOTICE 📢",
	"Before activating the options below, you must reset your skin; otherwise, you will keep dying infinitely."
	})
...
TabNuke:AddSection("NOTICE 🚨")
TabNuke:AddParagraph({
"warning from medr0so_0",
"For those wondering ''why did the skybox disappear?'', it's because Voldex patched that feature, and I only noticed now because I hadn't played Roblox for a while."
})


local tabtp = Window:MakeTab({"Teleport ", "Map"})
tabtp:AddSection("Teleports ")
tabtp:AddDropdown({
    Title = "Teleports",
    Options = {
        "Square",
...
tabtp:AddParagraph({
    "attention: ",
    "To use 'Power Plant Hideout', the power plant needs to be spawned, otherwise you'll go to the void. "
})


local tabscripts = Window:MakeTab({"Scripts", "Paper"})
tabscripts:AddSection("scripts for brookhaven")
...
Tabcasa:AddSection({ Name = "TROLL HOUSE" })
...
pcall(function()
    NoclipDoor = Tabcasa:AddToggle({
        Name = "GO THROUGH THE DOORS OF THE HOUSES",
        Description = "",
        Default = false,
        Callback = function(Value)
            pcall(function()
...
pcall(function()
    Tabcasa:AddSection({ Name = "TELEPORT TO HOMES" })
end)

-- Lista de casas para teletransporte
local casas = {
    ["Home 1"] = Vector3.new(260.29, 4.37, 209.32),
    ["Home 2"] = Vector3.new(234.49, 4.37, 228.00),
    ["Home 3"] = Vector3.new(262.79, 21.37, 210.84),
    ["Home 4"] = Vector3.new(229.60, 21.37, 225.40),
    ["Home 5"] = Vector3.new(173.44, 21.37, 228.11),
    ["Home 6"] = Vector3.new(-43, 21, -137),
    ["Home 7"] = Vector3.new(-40, 36, -137),
    ["Home 11"] = Vector3.new(-21, 40, 436),
    ["Home 12"] = Vector3.new(155, 37, 433),
    ["Home 13"] = Vector3.new(255, 35, 431),
    ["Home 14"] = Vector3.new(254, 38, 394),
    ["Home 15"] = Vector3.new(148, 39, 387),
    ["Home 16"] = Vector3.new(-17, 42, 395),
    ["Home 17"] = Vector3.new(-189, 37, -247),
    ["Home 18"] = Vector3.new(-354, 37, -244),
    ["Home 19"] = Vector3.new(-456, 36, -245),
    ["Home 20"] = Vector3.new(-453, 38, -295),
    ["Home 21"] = Vector3.new(-356, 38, -294),
    ["Home 22"] = Vector3.new(-187, 37, -295),
    ["Home 23"] = Vector3.new(-410, 68, -447),
    ["Home 24"] = Vector3.new(-348, 69, -496),
    ["Home 28"] = Vector3.new(-103, 12, 1087),
    ["Home 29"] = Vector3.new(-730, 6, 808),
    ["Home 30"] = Vector3.new(-245, 7, 822),
    ["Home 31"] = Vector3.new(639, 76, -361),
    ["Home 32"] = Vector3.new(-908, 6, -361),
    ["Home 33"] = Vector3.new(-111, 70, -417),
    ["Home 34"] = Vector3.new(230, 38, 569),
    ["Home 35"] = Vector3.new(-30, 13, 2209)
}
...
RGBTab:AddParagraph({
	"NOTICE 📢",
	"The options below were patched, so it was necessary to remove them."
})
...
local protec = Window:MakeTab({"Protection", "shield"})
protec:AddSection("Protections")
